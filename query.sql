...
DROP TABLE IF EXISTS Inpscripcion;
...
CREATE TABLE IF NOT EXISTS Inpscripcion(
    id INT UNSIGNED NOT NULL,
    fk_estudiante_id INT UNSIGNED NOT NULL,
    fk_curso_id INT UNSIGNED NOT NULL
);

ALTER TABLE Inpscripcion ADD CONSTRAINT PK_Inscripcion_id PRIMARY KEY (id);
...
ALTER TABLE Inpscripcion
    ADD CONSTRAINT FK_Inscripcion_Estudiante FOREIGN KEY (fk_estudiante_id) REFERENCES Estudiante(id),
    ADD CONSTRAINT FK_Inscripcion_Curso FOREIGN KEY (fk_curso_id) REFERENCES Curso(id);
...
INSERT INTO Inpscripcion VALUES
(1,51241,1),
(2,1312312,2),
(3,1312313,3),
(4,1312314,4),
...
-- Agrupa Cantidad de examenes de un curso
...
/* Muestra el nombre del curso su departamenot y nombre del profesor a los cursos con capacidad menor 40 */
SELECT 
    c.titulo as Curso, 
    d.nombre_departamento as Departamento, 
    CONCAT(p.nombres,' ',p.apellidos) as Profesor 
...
/* Muestre los estudiantes y su curso */
...
/* Muestra los Estudiantes Aprobados gracias al examen */
...
/* Departamento */
-- Muestra la informacion de un departamento
SELECT * FROM Departamento WHERE id = 3;

-- Muestra los departamentos que pertenecen a una facultad 
SELECT * FROM Departamento WHERE fk_Facultad_id = 1;

/* Pariente */
-- Muestra la informacion de un pariente 
SELECT * FROM Pariente WHERE id = 1225689741;

--Muestra el pariente asociado a un estudiante
SELECT * FROM Pariente WHERE id IN (SELECT pariente_id FROM Estudiante WHERE id = 51241);

/* Profesores*/ 

-- Muestra la informacion de un profesor
SELECT * FROM Profesores WHERE id = 100653;

-- Muestra todos los profesores que pertenecen a un departamento
